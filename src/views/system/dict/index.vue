...
const pageQueryParams = reactive<PageQuery<DictTypeQuery>>({
  page: 1,
  pageSize: 10,
  params: queryParams
});
...
function handleQuery() {
  loading.value = true;
  getDictTypePage(pageQueryParams)
    .then(({ data }) => {
...
const dictDataDialog = reactive<DialogType>({
  visible: false,
});
...
<template>
  <div class="app-container">
    ...
    <el-card shadow="never">
      <template #header>
        <el-button v-hasPerm="['sys:dict_type:add']" type="primary" @click="openDialog()"><i-ep-plus />新增</el-button>
        <el-button type="danger" v-hasPerm="['sys:dict_type:del']" :disabled="ids.length === 0"
      ...
      <el-table v-loading="loading" highlight-current-row :data="dictTypeList" border
        ...
        <el-table-column fixed="right" label="操作" align="center" width="220">
          <template #default="scope">
            ...
            <el-button v-hasPerm="['sys:dict_type:del']" type="primary" link size="small"
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" size="40%" @close="closeDialog">
    ...
    <el-dialog v-model="dictDataDialog.visible" :title="dictDataDialog.title" width="1000px" @close="closeDictDialog">
      <dict-data v-model:code="selectedDictType.typeCode" v-model:typeName="selectedDictType.typeName" />
    </el-dialog>
  </div>
</template>
